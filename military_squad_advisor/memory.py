...
class ConversationMemory:
    ...
    def get_role_specific_context(self, role, limit=10):
        ...
        Args:
            role: The squad role requesting context (leader, tactical, medic, scout, comms)
            limit: Maximum number of messages to include
        ...
        for msg in history:
            ...
            elif role == "comms" and any(keyword in msg["content"].lower() 
                                      for keyword in ["communicate", "talk", "negotiate", "discuss", "conversation", "message"]):
                # Communications specialist focuses on interaction patterns and negotiation
                context += f"[KEY INTERACTION] {msg['speaker']}: {msg['content']}\n\n"
            else:
